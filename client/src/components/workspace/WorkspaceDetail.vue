    ...
    <div class="content-body">
      ...
      <div class="section">
        <div class="section-header clickable" @click="toggleSection('basic')">
          <h3>{{ t.ws_basic_info }}</h3>
          <span class="fold-icon" :class="{ rotated: !visibility.basic }">▼</span>
        </div>
        <div class="section-content" v-if="visibility.basic">
          <div class="info-grid">
            <div class="info-item"><label>{{ t.ws_creator }}</label> <span>{{ info.creator }}</span></div>
            <div class="info-item"><label>{{ t.ws_created_date }}</label> <span>{{ info.createdDate }}</span></div>
          </div>
        </div>
      </div>
        ...
        <div class="section-content" v-if="visibility.approval">
          <div class="grid-container">
            <table class="grid-table">
              <thead>
                <tr>
                  <th width="100">{{ t.label_type }}</th>
                  <th>{{ t.ws_approver }}</th>
                  <th width="150">{{ t.ws_app_status }}</th>
                  <th width="200">{{ t.ws_app_date }}</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="step in info.approvals" :key="step.step">
                  ...
                  <td>
                    <span class="status-badge" :class="step.status.toLowerCase()">
                      {{ step.status === 'Completed' ? (currentLang === 'KO' ? '결재 완료' : 'Completed') : (currentLang === 'KO' ? '대기 중' : 'Pending') }}
                    </span>
                  </td>
        ...
        <div class="section-content" v-if="visibility.members">
          <div class="grid-container">
            <table class="grid-table">
              <thead>
                <tr>
                  <th width="150">{{ t.label_name }}</th>
                  <th width="200">{{ t.label_dept }}</th>
                  <th>{{ t.ws_role }}</th>
                  <th>{{ t.ws_email }}</th>
                </tr>
              </thead>
        ...
        <div class="section-content" v-if="visibility.models">
          <div class="grid-container">
            <table class="grid-table">
              <thead>
                <tr>
                  <th width="120">{{ t.ws_model_code }}</th>
                  <th>{{ t.ws_model_name }}</th>
                  <th width="120">과제코드</th>
                  <th>{{ t.ws_project }}</th>
                  <th width="100">{{ t.ws_project_pl }}</th>
                  <th width="120">{{ t.ws_app_status }}</th>
                </tr>
              </thead>
              <tbody>
    ...
    <div class="modal-overlay" v-if="selectedModel" @click.self="selectedModel = null">
      <div class="modal-content">
        <div class="modal-header">
          <h3>{{ t.ws_model_info }} ({{ selectedModel.code }})</h3>
          <button @click="selectedModel = null" class="close-btn">&times;</button>
        </div>
        <div class="modal-body">
          <div class="modal-info-grid">
            <div class="info-field"><label>{{ t.ws_model_code }}</label> <span>{{ selectedModel.code }}</span></div>
            <div class="info-field"><label>{{ t.ws_model_name }}</label> <span>{{ selectedModel.name }}</span></div>
            <div class="info-field"><label>과제 코드</label> <span>{{ selectedModel.projectCode }}</span></div>
            <div class="info-field"><label>{{ t.ws_project }}</label> <span>{{ selectedModel.projectName }}</span></div>
            <div class="info-field"><label>{{ t.ws_project_pl }}</label> <span>{{ selectedModel.projectPL }}</span></div>
            <div class="info-field"><label>{{ t.ws_app_status }}</label> <span>{{ selectedModel.status }}</span></div>
          </div>
          <div class="info-section">
            <label>{{ t.ws_description }}</label>
...
.section-header { display: flex; align-items: center; justify-content: space-between; padding: 14px 24px; background-color: var(--soft-blue); cursor: pointer; transition: all 0.2s; }
